...
class TxtGigSetlistRequestHandler(RequestHandler):
    ...
    def handle_get(self):
        ...
        txt = response.template('gig_setlist_download.txt',
                                 gig=gig,
                                 scores=scores)

        setlist_filename = 'Setlist {0} {1}.txt'.format(
            str(gig), filter.datetuple(*gig.DateTuple))

        return response.txt(txt.encode('utf-8-sig'), setlist_filename)
